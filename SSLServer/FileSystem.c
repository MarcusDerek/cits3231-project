...
void addFileToDirectory(char* file, char* username)
{
    ...
    strcat(destination,username); /* destination of the file that needs to be transferred Storage/Filename.txt */
    
    FILE * create;

    creation = destination; /* the files that needs to be replaced */
    strcat(creation,"/"); //Storage/username/
    strcat(creation,source); //Storage/username/filename.txt

    
    create = fopen(creation,"wb");
    
    if ( rename(file,creation) ) //filenamt.txt /Storage/username/fileName.txt
    {
